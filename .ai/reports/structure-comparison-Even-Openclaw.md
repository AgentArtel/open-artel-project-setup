...
**Current Project**: open-artel-project-setup
...
| Review feedback directory (`.ai/reviews`) | Yes |
| Reports directory (`.ai/reports`) | Yes |
| Instructions directory (`.ai/instructions`) | Yes |
| Chat logs directory (`.ai/chats`) | Yes |
...
| Orchestrator config (`CLAUDE.md`) | Yes |
...
- Past config has `CURSOR_WORKFORCE.md` (workforce protocol) — current project does not
...
- Task count differs: current has 3, past had 23
...
- **Missing workforce protocol**: Past config had `CURSOR_WORKFORCE.md` with Manager/Task Chat setup. Consider adopting.
